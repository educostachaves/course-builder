...
// Import Actions
import { addCourseRequest, fetchCourses, deleteCourseRequest } from '../../CourseActions';
...
class CourseListPage extends Component {
  ...
  handleAddCourse = (name, subtitle, description, price, duration) => {
    this.props.dispatch(toggleAddCourse());
    this.props.dispatch(addCourseRequest({ name, subtitle, description, price, duration }));
  };

  render() {
    return (
      <div>
        <CourseCreateWidget addCourse={this.handleAddCourse} showAddCourse={this.props.showAddCourse} />
        <CourseList handleDeleteCourse={this.handleDeleteCourse} courses={this.props.courses} />
      </div>
    );
  }
}
